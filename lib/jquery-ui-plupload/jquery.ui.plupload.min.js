...
this.element.attr("id",a));this.id=a;this.contents_bak=this.element.html();var b=this.element;b.id=b.attr("id");b.html('<div class="plupload_wrapper"><div class="plupload_container"><div class="plupload_content"><div class="plupload_droptext">'+g("Drag files here")+'</div><ul class="plupload_filelist_content"> </ul><div class="plupload_clearer">&nbsp;</div></div><table class="plupload_filelist plupload_filelist_footer"><tr><td class="plupload_cell plupload_file_name"><div class="plupload_buttons">\x3c!-- Visible --\x3e<a class="plupload_button plupload_add">'+
...
a+"_stop");this.cancel_button=c(".plupload_cancel",this.container).attr("id",a+"_cancel");this.thumbs_switcher=c("#"+a+"_view_thumbs");this.list_switcher=c("#"+a+"_view_list");c.ui.button&&(this.browse_button.uibutton({icons:{primary:"ui-icon-circle-plus"},disabled:!0}),this.start_button.uibutton({icons:{primary:"ui-icon-circle-arrow-e"},disabled:!0}),this.stop_button.uibutton({icons:{primary:"ui-icon-circle-close"}}),this.cancel_button.uibutton({icons:{primary:"ui-icon-circle-close"}}),this.list_switcher.uibutton({text:!1,
icons:{secondary:"ui-icon-grip-dotted-horizontal"}}),this.thumbs_switcher.uibutton({text:!1,icons:{secondary:"ui-icon-image"}}));this.progressbar=c(".plupload_progress_container",this.container);c.ui.progressbar&&this.progressbar.progressbar();this.counter=c(".plupload_count",this.element).attr({id:a+"_count",name:a+"_count"});this._initUploader()},_initUploader:function(){var a=this,b=this.id,f={container:b+"_buttons",browse_button:b+"_browse"};c(".plupload_buttons",this.element).attr("id",b+"_buttons");
if(a.options.dragdrop){var d=this.filelist.parent();d.attr("id",this.id+"_dropbox");f.drop_element=this.id+"_dropbox";d.on({dragenter:function(a){c(this).addClass("drop_active")},dragleave:function(a){c(this).removeClass("drop_active")},drop:function(a){c(this).removeClass("drop_active");c(this).find(".plupload_droptext").remove()}})}this.filelist.on("click",function(b){c(b.target).hasClass("plupload_action_icon")&&(a.removeFile(c(b.target).closest(".plupload_file").attr("id")),b.preventDefault())});
b=this.uploader=t[b]=new e.Uploader(c.extend(this.options,f));a.options.views.thumbs&&(b.settings.required_features.display_media=!0);a.options.max_file_count&&e.extend(b.getOption("filters"),{max_file_count:a.options.max_file_count});e.addFileFilter("max_file_count",function(b,c,e){b<=this.files.length-(this.total.uploaded+this.total.failed)?(a.browse_button.uibutton("disable"),this.disableBrowse(),this.trigger("Error",{code:a.FILE_COUNT_ERROR,message:g("File count error."),file:c}),e(!1)):e(!0)});
...
b.getOption("filters").max_file_count||0);break;case e.IMAGE_FORMAT_ERROR:d=g("Image format either wrong or not supported.");break;case e.IMAGE_MEMORY_ERROR:d=g("Runtime ran out of available memory.");break;case e.HTTP_ERROR:d=g("Upload URL might be wrong or doesn't exist.")}f+=" <br /><i>"+d+"</i>";a._trigger("error",null,{up:b,error:c});c.code===e.INIT_ERROR?setTimeout(function(){a.destroy()},1):a.notify("error",f)});b.bind("PostInit",function(b){a.options.buttons.browse?a.browse_button.uibutton("enable"):
(a.browse_button.uibutton("disable").hide(),b.disableBrowse(!0));a.options.buttons.start||a.start_button.uibutton("disable").hide();a.options.buttons.stop||a.stop_button.uibutton("disable").hide();a.options.dragdrop&&b.features.dragdrop&&a.filelist.parent().addClass("plupload_dropbox");a._enableViewSwitcher();a.start_button.click(function(b){c(this).uibutton("option","disabled")||a.start();b.preventDefault()});a.stop_button.click(function(b){a.stop();c("#vls-gf-upload-panel").slideUp();a.clearQueue();
b.preventDefault()});a.cancel_button.click(function(b){a.clearQueue();b.preventDefault()});a._trigger("ready",null,{up:b})});b.init();b.bind("FileFiltered",function(b,c){a._addFiles(c)});b.bind("FilesAdded",function(b,d){a._trigger("selected",null,{up:b,files:d});a.options.sortable&&c.ui.sortable&&a._enableSortingList();a._trigger("updatelist",null,{filelist:a.filelist});a.options.autostart&&setTimeout(function(){a.start()},10)});b.bind("FilesRemoved",function(b,d){c.ui.sortable&&a.options.sortable&&
c("tbody",a.filelist).sortable("destroy");c.each(d,function(a,b){c("#"+b.id).toggle("highlight",function(){c(this).remove()})});b.files.length&&a.options.sortable&&c.ui.sortable&&a._enableSortingList();a._trigger("updatelist",null,{filelist:a.filelist});a._trigger("removed",null,{up:b,files:d})});b.bind("QueueChanged StateChanged",function(){a._handleState()});b.bind("UploadFile",function(b,c){a._handleFileStatus(c)});b.bind("FileUploaded",function(b,c){a._handleFileStatus(c);a._trigger("uploaded",
null,{up:b,file:c})});b.bind("UploadProgress",function(b,c){a._handleFileStatus(c);a._updateTotalProgress();a._trigger("progress",null,{up:b,file:c})});b.bind("UploadComplete",function(b,c){a._addFormFields();a._trigger("complete",null,{up:b,files:c})})},_setOption:function(a,b){"buttons"==a&&"object"==typeof b&&(b=c.extend(this.options.buttons,b),b.browse?(this.browse_button.uibutton("enable").show(),this.uploader.disableBrowse(!1)):(this.browse_button.uibutton("disable").hide(),this.uploader.disableBrowse(!0)),
b.start?this.start_button.uibutton("enable").show():this.start_button.uibutton("disable").hide(),b.stop?this.start_button.uibutton("enable").show():this.stop_button.uibutton("disable").hide());this.uploader.settings[a]=b},start:function(){this.uploader.start();this._trigger("start",null,{up:this.uploader})},stop:function(){this.uploader.stop();this._trigger("stop",null,{up:this.uploader})},enable:function(){this.browse_button.uibutton("enable");this.uploader.disableBrowse(!1)},disable:function(){this.browse_button.uibutton("disable");
this.uploader.disableBrowse(!0)},getFile:function(a){return"number"===typeof a?this.uploader.files[a]:this.uploader.getFile(a)},getFiles:function(){return this.uploader.files},removeFile:function(a){"string"===e.typeOf(a)&&(a=this.getFile(a));this.uploader.removeFile(a)},clearQueue:function(){this.uploader.splice()},getUploader:function(){return this.uploader},refresh:function(){this.uploader.refresh()},notify:function(a,b){var f=c('<div class="plupload_message"><span class="plupload_message_close ui-icon ui-icon-circle-close" title="'+
g("Close")+'"></span><p><span class="ui-icon"></span>'+b+"</p></div>");f.addClass("ui-state-"+("error"===a?"error":"highlight")).find("p .ui-icon").addClass("ui-icon-"+("error"===a?"alert":"info")).end().find(".plupload_message_close").click(function(){f.remove()}).end();c(".plupload_header",this.container).append(f)},destroy:function(){this.uploader.destroy();c(".plupload_button",this.element).unbind();c.ui.button&&c(".plupload_add, .plupload_start, .plupload_stop",this.container).uibutton("destroy");
c.ui.progressbar&&this.progressbar.progressbar("destroy");c.ui.sortable&&this.options.sortable&&c("tbody",this.filelist).sortable("destroy");this.element.empty().html(this.contents_bak);this.contents_bak="";c.Widget.prototype.destroy.apply(this)},_handleState:function(){var a=this.uploader,b=a.files.length-(a.total.uploaded+a.total.failed),f=a.getOption("filters").max_file_count||0;e.STARTED===a.state?(c([]).add(this.stop_button).add(".plupload_started").removeClass("plupload_hidden"),this.start_button.uibutton("disable"),
...
this.cancel_button.uibutton("enable").show(),this.options.multiple_queues&&c(".plupload_header_content",this.element).removeClass("plupload_header_content_bw"),this.options.multiple_queues&&f&&f>b&&(this.browse_button.uibutton("enable"),a.disableBrowse(!1)),this._updateTotalProgress());a.refresh()},_handleFileStatus:function(a){var b=c("#"+a.id),f,d;if(b.length){switch(a.status){case e.DONE:f="plupload_done";d="plupload_action_icon ui-icon ui-icon-circle-check";break;case e.FAILED:f="ui-state-error plupload_failed";
