<div class="vls-gf-column-2">
    ...
    <div class="vls-gf-info-line">
        <strong><?php _e('Uploaded by:', VLS_GF_TEXTDOMAIN); ?></strong><?php echo get_the_author_meta('nicename', $image->post_author); ?>
    </div>
</div>
<div class="vls-gf-column-2">
    <?php if ('' != $image->camera) { ?>
    ...
    if ('' != $image->focal_length) { ?>
        <div class="vls-gf-info-line">
            <strong><?php _e('Focal length:', VLS_GF_TEXTDOMAIN); ?></strong><?php echo $image->focal_length; ?>
        </div>
    <?php }
    if ('' != $image->shutter_speed) { ?>
        <div class="vls-gf-info-line">
            <strong><?php _e('Shutter speed:', VLS_GF_TEXTDOMAIN); ?></strong><?php echo $image->shutter_speed; ?>
        </div>
    <?php }
    if ('' != $image->aperture) { ?>
        <div class="vls-gf-info-line">
